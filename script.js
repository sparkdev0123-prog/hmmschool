    ...
    document.querySelectorAll(".js-form").forEach((form) => {
      form.addEventListener("submit", (event) => {
        ...
        if (form.dataset.emailForm === "admission") {
          const formData = new FormData(form);
          const lines = [
            "Admission enquiry for Jack N Jill Public School",
            "",
            `Student Name: ${formData.get("studentName") || ""}`,
            `Parent Name: ${formData.get("parentName") || ""}`,
            `Mobile Number: ${formData.get("mobileNumber") || ""}`,
            `Class Applying For: ${formData.get("classApplying") || ""}`,
            "",
            "Message:",
            formData.get("message") || ""
          ];
          const subject = encodeURIComponent("Admission Enquiry - Jack N Jill Public School");
          const body = encodeURIComponent(lines.join("\n"));
          if (status) {
            status.textContent = "Opening your email app with the enquiry details.";
          }
          window.location.href = `mailto:jacknjillpublicschool@gmail.com?subject=${subject}&body=${body}`;
          form.reset();
          return;
        }
        if (status) {
